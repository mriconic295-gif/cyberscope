...
class Dashboard(QWidget):
    ...
    def build_ui(self):
        ...
        self.targetCard = InfoCard("Target", "--")
        self.ipCard = InfoCard("Real IP", "--")
        self.riskCard = InfoCard("Risk Score", "0 / 100")
        self.responseCard = InfoCard("Response Time", "--")

        cardLayout.addWidget(self.targetCard, 0, 0)
        cardLayout.addWidget(self.ipCard, 0, 1)
        cardLayout.addWidget(self.riskCard, 0, 2)
        cardLayout.addWidget(self.responseCard, 0, 3)

        root.addLayout(cardLayout)
        ...
        # LEFT PANEL
        # --------------------------------------------------
        self.infoPanel = QGroupBox("Target Information")
        ...
        # CENTER PANEL
        # --------------------------------------------------
        self.screenshotPanel = QGroupBox("Website Screenshot")
        ...
        shotLayout.addWidget(self.screenshotLabel)

        # --------------------------------------------------
        # RIGHT PANEL
        # --------------------------------------------------
        self.techPanel = QGroupBox("Detected Technologies")
        techLayout = QVBoxLayout(self.techPanel)

        self.techText = QTextEdit()
        self.techText.setReadOnly(True)
        techLayout.addWidget(self.techText)

        self.grid.addWidget(self.infoPanel, 0, 0)
        self.grid.addWidget(self.screenshotPanel, 0, 1)
        self.grid.addWidget(self.techPanel, 0, 2)
        ...
        self.ai_summary.setPlaceholderText("AI analysis will appear here...")
        aiLayout.addWidget(self.ai_summary)

        # TERMINAL
        self.terminalPanel = QGroupBox("Live Scanner Log")
        terminalLayout = QVBoxLayout(self.terminalPanel)
        self.terminal = QTextEdit()
        self.terminal.setReadOnly(True)
        self.terminal.setPlaceholderText("[+] Waiting for scan...")
        terminalLayout.addWidget(self.terminal)

        # DNS
        self.dnsPanel = QGroupBox("DNS Records")
        dnsLayout = QVBoxLayout(self.dnsPanel)
        self.dnsText = QTextEdit()
        self.dnsText.setReadOnly(True)
        dnsLayout.addWidget(self.dnsText)

        # WHOIS
        self.whoisPanel = QGroupBox("WHOIS")
        whoisLayout = QVBoxLayout(self.whoisPanel)
        self.whoisText = QTextEdit()
        self.whoisText.setReadOnly(True)
        whoisLayout.addWidget(self.whoisText)

        # SSL
        self.sslPanel = QGroupBox("SSL Certificate")
        sslLayout = QVBoxLayout(self.sslPanel)
        self.sslText = QTextEdit()
        self.sslText.setReadOnly(True)
        sslLayout.addWidget(self.sslText)

        # SECURITY HEADERS
        self.securityPanel = QGroupBox("Security Headers")
        securityLayout = QVBoxLayout(self.securityPanel)
        self.securityText = QTextEdit()
        self.securityText.setReadOnly(True)
        securityLayout.addWidget(self.securityText)

        # HTTP HEADERS
        self.headersPanel = QGroupBox("HTTP Headers")
        headersLayout = QVBoxLayout(self.headersPanel)
        self.headersText = QTextEdit()
        self.headersText.setReadOnly(True)
        headersLayout.addWidget(self.headersText)

        # GRID MAPPING
        bottomLayout.addWidget(self.aiPanel, 0, 0)
        bottomLayout.addWidget(self.terminalPanel, 0, 1)
        bottomLayout.addWidget(self.dnsPanel, 1, 0)
        bottomLayout.addWidget(self.whoisPanel, 1, 1)
        bottomLayout.addWidget(self.sslPanel, 2, 0)
        bottomLayout.addWidget(self.securityPanel, 2, 1)
        bottomLayout.addWidget(self.headersPanel, 3, 0, 1, 2)
    ...
    def set_response_time(self, value):
        self.responseCard.setValue(f"{value} ms")

    def update_score(self, score):
        self.riskCard.setValue(f"{score}/100")
    ...
    def set_technologies(self, data):
        if isinstance(data, list):
            ...
            self.techText.setPlainText(str(data))

    def set_dns(self, data):
        self.dnsText.setPlainText(str(data))

    def set_whois(self, data):
        self.whoisText.setPlainText(str(data))

    def set_ssl(self, data):
        self.sslText.setPlainText(str(data))

    def set_headers(self, data):
        self.headersText.setPlainText(str(data))

    def set_security_headers(self, data):
        self.securityText.setPlainText(str(data))

    def set_ai_summary(self, text):
        self.ai_summary.setPlainText(text)
    ...
    def clear(self):
        ...
        self.terminal.clear()
        self.screenshotLabel.setText("Screenshot Preview")
    ...
    def set_performance(self, data):
        self.log(f"[+] Performance : {data}")

    def set_favicon(self, path):
        self.log("[+] Favicon Loaded")
