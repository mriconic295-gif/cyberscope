...
class HTTPSession:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            "User-Agent": "CyberScope Recon Engine"
        })
        self.timeout = 10
...
class ReconnaissanceEngine:
    ...
    def run(self, target):
        ...
        if not info["valid"]:
            result["errors"].append("Invalid or unreachable target.")
            return result
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = {
                "dns": executor.submit(self.dns.lookup, hostname),
                "ip": executor.submit(self.ip.lookup, hostname),
                "whois": executor.submit(self.whois.lookup, hostname),
                "performance": executor.submit(self.performance.analyze, url),
                "response": executor.submit(self.response.measure, url),
                "host": executor.submit(self.host.collect, hostname)
            }
        ...
        if ip:
            try:
                result["data"]["reverse_dns"] = self.reverse.lookup(ip)
            except Exception:
                result["data"]["reverse_dns"] = None

            try:
                result["data"]["geoip"] = self.geo.lookup(ip)
            except Exception:
                result["data"]["geoip"] = None

            try:
                result["data"]["asn"] = self.asn.lookup(ip)
            except Exception:
                result["data"]["asn"] = None

            try:
                result["data"]["ports"] = self.port.scan(ip)
            except Exception:
                result["data"]["ports"] = {}
...
if __name__ == "__main__":
    ...
    print(ResultFormatter.pretty(result))
